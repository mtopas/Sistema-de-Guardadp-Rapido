...
import { buildRegistrosMap, calcStreak, calcMonthPct, isScheduled, toISODate } from './habitosUtils'
import HabitosRightPanel from './HabitosRightPanel'

const MONTH_NAMES = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic']

function getMomentumMsg(pctThisWeek, pctLastWeek, streakMax) {
...
export default function ProgresoTab({ selectedId, setSelectedId, onEdit }) {
  ...
  const pctThisWeek  = calcWeekPct(activos, registrosMap, 0)
  const pctLastWeek  = calcWeekPct(activos, registrosMap, 1)
  const pctThisMonth = (() => {
  ...
  const momentum = getMomentumMsg(pctThisWeek, pctLastWeek, 0)
  ...
  for (let i = 83; i >= 0; i--) {
    ...
    heatmapDays.push({ dateStr, pct, d })
  }
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    sparkData.push({ label: MONTH_NAMES[m], pct: s > 0 ? Math.round((done / s) * 100) : 0 })
  }

  // Habit stats table
  const habitStats = activos.map(h => {
    const streak    = calcStreak(h, registrosMap)
    const pctMes    = calcMonthPct(h, registrosMap, today.getFullYear(), today.getMonth())
    const allDone   = habitosRegistros.filter(r => r.habito_id === h.id && r.valor > 0)
    let maxStr = 0, cur = 0
    const dates = allDone.map(r => r.fecha).sort()
    for (let i = 0; i < dates.length; i++) {
      if (i === 0) { cur = 1; maxStr = 1; continue }
    ...
    const pctLast = calcMonthPct(h, registrosMap, today.getFullYear(), today.getMonth() - 1 < 0 ? 11 : today.getMonth() - 1)
      ...
      <div className="flex-1 min-w-0 overflow-y-auto p-5 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Vs sem. ant.', value: pctLastWeek, diff: null, sub: 'semana anterior' },
          ].map((s, i) => (
        ...
        <div
          ...
          style={{
            ...
            color: momentum.type === 'success' ? 'var(--success)'
              ...
              : 'var(--subtext)',
            border: `1px solid color-mix(in oklch, ${momentum.type === 'success' ? 'var(--success)' : momentum.type === 'warning' ? 'var(--warning)' : 'var(--border)'} 30%, transparent)`,
          }}
        >
          {momentum.text}
        </div>
