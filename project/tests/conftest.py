...
IMPORTANTE: `DB_PATH` se lee una sola vez, a nivel de módulo, en `app.config`
(`app/db/database.py` hace `from app.config import DEBUG, DB_PATH` en su import).
Por eso el override de env var tiene que pasar ANTES de que cualquier test o
...
pasar, así que el `os.environ[...]` va arriba de todo, antes de cualquier `import app`.

Nunca apunta a `project/database/app.db` real: siempre un archivo de scratch en
%TEMP% (o equivalente), descartable entre tests.
"""
import os
import tempfile
